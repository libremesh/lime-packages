...
local function mock_last_internet_path(data)
	...
	stub(metrics, "get_last_internet_path_filename", function()
		return fake_path
	end)
...
describe("Lime-metric tests", function()
	...
	it("test get_gateway return last entry on the last internet file", function()
		mock_last_internet_path(
			'[{"ip":"10.133.43.6", "hostname":"node_foo"}, ' .. '{"ip":"10.0.0.1","hostname":"thegateway"}]'
		)
	...
	it("test get_station_traffic", function()
		stub(lutils, "unsafe_shell", function()
			return nil
		end)
		stub(lutils, "unsafe_shell", function()
			return "256723649\n22785424"
	...
	it("test get_metrics no link", function()
		stub(lutils, "is_installed", function(m)
			return m == "lime-proto-babeld"
		end)
	...
	it("test get_internet_status with internet", function()
		stub(utils, "get_loss", function()
			return "25"
		end)
		stub(utils, "is_nslookup_working", function()
			return true
		end)
