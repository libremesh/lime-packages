...
function smart_wifi.run()
	...
	for _, radio in pairs(all_radios) do
		...
		config.set(radioName, "channel", channels[freq][channels_active[freq]])
	...
	if multiBand then
		for _, radio in pairs(all_radios) do
			...
			local modes_radio = {}
			if is5ghz then
				if config.get_bool("smart_wifi", "mesh_5ghz", 1) or config.get_bool("smart_wifi", "all_to_mesh", 0) then
					print("set 5Ghz radio " .. radioName .. " to mesh")
					modes_radio = smart_wifi.add_mesh(modes_radio)
				end
				if config.get_bool("smart_wifi", "ap_5ghz", 0) or config.get_bool("smart_wifi", "all_to_ap", 0) then
					print("set 5Ghz radio " .. radioName .. " to ap")
					modes_radio = smart_wifi.add_ap(modes_radio)
				end
			else
				if
					(config.get_bool("smart_wifi", "ap_2ghz", 1) and not added_2ghz_ap)
					or config.get_bool("smart_wifi", "all_to_ap", 0)
				then
					print("set 2.4Ghz radio " .. radioName .. " to ap")
					modes_radio = smart_wifi.add_ap(modes_radio)
					added_2ghz_ap = true
				end
				if config.get_bool("smart_wifi", "mesh_2ghz", 0) or config.get_bool("smart_wifi", "all_to_mesh", 0) then
