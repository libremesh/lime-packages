...
local shared_state_output_text =
	require("./packages/shared-state-wifi_links_info/tests/shared_state_wifi_links_info_output_text")
...
it("a simple test to get links info and assert requiered fields are present", function()
	...
	stub(node_status, "get_stations", function()
		return iwinfo.mocks.get_stations
	end)
	stub(node_status, "get_stations", function()
		return iwinfo.mocks.get_stations
	end)
	stub(iwinfo.nl80211, "frequency", function()
		return 2400
	end)
