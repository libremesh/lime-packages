...
function config.get_bool(sectionname, option, default)
	...
	return (
		val and ((val == "1") or (val == "on") or (val == "true") or (val == "enabled") or (val == 1) or (val == true))
	)
end
...
function config.set(...)
	local args = { ... }
	local aty = type(args[3])
	if aty ~= "nil" and aty ~= "string" and aty ~= "table" then
		args[3] = tostring(args[3])
	end
	config.uci:set(config.UCI_CONFIG_NAME, unpack(args))
	if not config.batched then
...
function config.delete(...)
	local args = { ... }
	config.uci:delete(config.UCI_CONFIG_NAME, unpack(args))
	if not config.batched then
		config.uci:save(config.UCI_CONFIG_NAME)
	end
end
