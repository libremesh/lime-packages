...
function utils.dbg(...)
	...
	ofd:write(debug.getinfo(2, "S").source, ":", debug.getinfo(2, "l").currentline, " ", debug.getinfo(2, "n").name)
...
function utils.log(...)
	if DISABLE_LOGGING ~= nil then
		return
	end
	if os.getenv("LUA_DISABLE_LOGGING") ~= nil and os.getenv("LUA_ENABLE_LOGGING") == nil then
		return
	end
	print(string.format(...))
end
...
function utils.literalize(str)
	local ret, _ = str:gsub("[%(%)%.%%%+%-%*%?%[%]%^%$]", function(c)
		return "%" .. c
	end)
	return ret
end
...
function utils.applyMacTemplate16(template, mac)
	...
	for i = 1, 6, 1 do
		template = template:gsub("%%m" .. i, macid[i])
	end
	return template
end
...
function utils.applyMacTemplate10(template, mac)
	...
	local macid = utils.get_id(mac)
	for i = 1, 6, 1 do
		template = template:gsub("%%m" .. i, tonumber(macid[i], 16))
	end
	return template
end
...
function utils.applyNetTemplate16(template)
	...
	for i = 1, 6, 1 do
		template = template:gsub("%%N" .. i, netid[i])
	end
	return template
end
...
function utils.applyNetTemplate10(template)
	local netid = utils.network_id()
	for i = 1, 6, 1 do
		template = template:gsub("%%N" .. i, tonumber(netid[i], 16))
	end
	return template
end
...
function utils.is_valid_hostname(hostname)
	if hostname and (#hostname < 64) and hostname:match("^[a-zA-Z0-9][a-zA-Z0-9%-]*[a-zA-Z0-9]$") then
		return true
	end
	return false
end
...
function utils.set_password(username, password)
	...
	return os.execute(string.format("(echo %s; sleep 1; echo %s) | passwd %s >/dev/null 2>&1", pass, pass, user))
end
...
function utils.http_client_get(url, timeout_s, out_file)
	...
	local cmd = string.format("uclient-fetch -q -O %s --timeout=%d %s 2> /dev/null", out_file, timeout_s, url)
