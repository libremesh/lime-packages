...
describe("pirania rpcd tests #piraniarpcd", function()
	...
	it("test portal_config", function()
		stub(portal, "get_config", function()
			return { activated = false }
		end)
		stub(portal, "set_config", function()
			return true
		end)
	...
	it("test list vouchers", function()
		local json_data = json.stringify({
			name = "foo",
			duration_m = 100,
			activation_deadline = os.time() + 10,
			permanent = false,
			qty = 5,
		})
	...
	it("test set_portal_page_content calls set_page_content", function()
		...
		assert
			.stub(portal.set_page_content)
			.was_called_with(msg.title, msg.main_text, msg.logo, msg.link_title, msg.link_url, msg.background_color)
	end)
