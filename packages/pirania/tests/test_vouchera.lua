...
describe("Vouchera tests #vouchera", function()
	...
	it("init and compare vouchers", function()
		...
		local voucher_f = vouchera.voucher({
			name = "myvoucher",
			code = "secret_code",
			mod_counter = 2,
			creation_date = current_time_s,
		})
		local voucher_g = vouchera.voucher({
			name = "myvoucher",
			code = "secret_code",
			mod_counter = 3,
			creation_date = current_time_s,
		...
		local voucher_h = vouchera.voucher({
			name = "myvoucher",
			code = "secret_code",
			id = "foo",
			duration_m = 100,
		...
		local voucher_i = vouchera.voucher({
			name = "myvoucher",
			code = "secret_code",
			id = "foo",
			duration_m = 100,
		...
		local voucher_j = vouchera.voucher({
			name = "myvoucher",
			code = "secret_code",
			id = "bar",
			duration_m = 100,
	...
	it("prune invalidated vouchers", function()
		...
		stub(os, "time", function()
			return pre_expiry_time
		end)
		assert.is_false(vouchera.should_be_pruned(voucher))
		stub(os, "time", function()
			return pre_expiry_time + 10
		end)
	...
	it("test automatic pruning of old voucher", function()
		...
		stub(os, "time", function()
			return current_time_s + (31 * 60 * 60 * 24)
		end)
		os.execute:revert()
		vouchera.init()
		assert.is_nil(vouchera.get_by_id("myvoucher"))
	...
	before_each("", function()
		...
		stub(os, "execute", function(args)
			print("Warn, os.execute called with: " .. args)
		end)
	end)
