...
local function voucher_init(obj)
	...
	voucher.tostring = function()
		...
		return (
			string.format(
				...
				v.name,
				v.code,
				v.mac or "xx:xx:xx:xx:xx:xx",
				creation,
				v.duration_m or "perm",
				expiration,
				v.mod_counter,
				unrestricted_str
			)
	...
	voucher.is_expired = function()
		...
		return (voucher.expiration_date() ~= nil and voucher.expiration_date() < curr_time)
			or (voucher.activation_deadline ~= nil and voucher.activation_deadline < curr_time)
	end
...
function vouchera.should_be_pruned(voucher)
	...
	return (
		...
		and (voucher.expiration_date() <= (current_time - vouchera.PRUNE_OLDER_THAN_S))
	)
		or (
			(voucher.invalidation_date or false)
			and (voucher.invalidation_date <= (current_time - vouchera.PRUNE_OLDER_THAN_S))
		)
end
...
function vouchera.gen_code()
	return utils.random_string(vouchera.CODE_SIZE, function(c)
