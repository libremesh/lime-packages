		...
		if utils.stringStarts(sectionName, usbradio.sectionNamePrefix) then
			...
			if config.get_bool(sectionName, "autogenerated") then
				...
				if numberOfMatches < 1 then
					uci:delete("wireless", sectionName)
...
function usbradio.detect_hardware()
	...
	for path in stdOutput:lines() do
		...
		local sectionName = usbradio.sectionNamePrefix.."radio"..phyIndex
		...
		if ( (not config.get_all(sectionName)) or config.get_bool(sectionName, "autogenerated") ) then
			...
			uci:set("wireless", sectionName, "wifi-device")
			uci:set("wireless", sectionName, "type", "mac80211")
			uci:set("wireless", sectionName, "channel", "11") --TODO: working on all 802.11bgn devices; find a general way for working in different devices
			uci:set("wireless", sectionName, "hwmode", "11ng") --TODO: working on all 802.11gn devices; find a general way for working in different devices
			uci:set("wireless", sectionName, "path", phyPath)
			uci:set("wireless", sectionName, "htmode", "HT20")
			uci:set("wireless", sectionName, "disabled", "0")
