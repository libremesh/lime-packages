...
fbw.FETCH_CONFIG_STATUS = {
	downloaded_config = {
		retval = true,
		code = "downloaded_config",
	},
	downloading_config = {
		retval = true,
		code = "downloading_config",
	},
	error_download_lime_community = {
		retval = false,
		code = "error_download_lime_community",
	},
	...
	error_download_lime_assets = {
		retval = false,
		code = "error_download_lime_assets",
	},
}
...
function fbw.get_networks()
	...
	local networks = ft.map(function(phy)
...
function fbw.get_own_macs()
	...
	local radios_5ghz = ft.filter(wireless.is5Ghz, radios)
	local phys = ft.map(utils.extract_phys_from_radios, radios_5ghz)
	return ft.map(function(phy)
		return table.concat(wireless.get_phy_mac(phy), ":")
	end, phys)
end
...
function fbw.fetch_lime_community_assets(host, lime_community_fname)
	local res =
		lutils.http_client_get("http://[" .. host .. "]/cgi-bin/lime/lime-community-assets", 10, lime_community_fname)
	return res
end
...
function fbw.fetch_config(data)
	...
	local hostname =
		utils.execute("wget --no-check-certificate http://[" .. data.host .. "]/cgi-bin/hostname -qO - "):gsub("\n", "")
