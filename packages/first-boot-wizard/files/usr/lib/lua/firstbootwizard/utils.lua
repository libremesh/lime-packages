...
local function split(str, sepArg)
	local sep, fields = sepArg or ":", {}
...
local find_network = function(network, data)
	return ft.filter(function(net)
		return same_network(net, network)
	end, data)
end
...
function utils.is_connected(dev_id)
	...
	while (tableEmpty(isAssociated)) and i < 5 do
		...
		if tableEmpty(isAssociated) == false then
			break
		end
		i = i + 1
		os.execute("sleep 2s")
