...
describe("ubus-shared-state tests #ubus-shared-state", function()
	before_each("", function()
		local testDir = testUtils.setup_test_dir()
	...
	it("test get data ", function()
		...
		local response = rpcdCall(sharedStateRpc, { "call", "getFromSharedState" }, '{"data_type": "wifi_links_info"}')
	...
	it("test get multiwriter data from empty data_type ", function()
		sharedState.SharedStateMultiWriter.new("EMPTY")

		local response = rpcdCall(sharedStateRpc, { "call", "getFromSharedStateMultiWriter" }, '{"data_type": "EMPTY"}')
	...
	it("test get multiwriter data ", function()
		...
		local response = rpcdCall(sharedStateRpc, { "call", "getFromSharedStateMultiWriter" }, '{"data_type": "A"}')
		assert.is.equal(response.data.bar, "foo")
		assert.is.equal(response.data.baz, "qux")
		assert.is.equal(response.data.zig, "zag")
	end)
	...
	it("test insert multiwriter data ", function()
		...
		rpcdCall(
			sharedStateRpc,
			{ "call", "insertIntoSharedStateMultiWriter" },
	...
	it("test insert multiwriter data ", function()
		...
		response.zig = "newzag"
		local callargs = '{"data_type": "A", "json": ' .. json.stringify(response) .. "}"
		rpcdCall(sharedStateRpc, { "call", "insertIntoSharedStateMultiWriter" }, callargs)
		local dbA = sharedStateA:get()
		assert.is.equal("foo", dbA.bar.data)
		assert.is.equal("newzag", dbA.zig.data)
	end)
	...
	it("test set multiwriter big chunk of data ", function()
		...
		rpcdCall(sharedStateRpc, { "call", "insertIntoSharedStateMultiWriter" }, wifiStatusJsonsample23)
		response = rpcdCall(
			sharedStateRpc,
			{ "call", "getFromSharedStateMultiWriter" },
		...
		rpcdCall(sharedStateRpc, { "call", "insertIntoSharedStateMultiWriter" }, wifiStatusJsonsample27)
