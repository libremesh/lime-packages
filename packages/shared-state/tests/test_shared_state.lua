...
describe('Shared State Persistent tests #sharedstate-persistent', function()
    before_each('', function()
        test_dir = test_utils.setup_test_dir()
    ...
    it('test merge data from remote db with newer timestamp', function()
        local sharedStateA = shared_state.SharedStatePersistent:new('A')
        local sharedStateB = shared_state.SharedStatePersistent:new('B')
        stub(os, "time", function () return 1616095000 end)
        sharedStateA:insert({ bar = 'foo', baz = 'qux', zig = 'old_zag'})
        stub(os, "time", function () return 1616096000 end)
    ...
    it('test merge data from remote db with older timestamp', function()
        local sharedStateA = shared_state.SharedStatePersistent:new('A')
        local sharedStateB = shared_state.SharedStatePersistent:new('B')
        stub(os, "time", function () return 1616096000 end)
