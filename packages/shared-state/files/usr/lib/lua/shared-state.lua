...
function SharedStateBase:lock(maxwait)
	...
	self.storageFD = nixio.open(self.dataFile, nixio.open_flags("rdwr", "creat"))
...
function SharedStateBase:notifyHooks()
	if self.changed then
		...
		if not fs.dir(self.hooksDir) then
			return
		end
...
function SharedStateBase:_sync(urls)
	...
	if #urls < 1 then
		...
		for _, line in pairs(fixed_candidates) do
			table.insert(urls, line .. "/" .. self.dataType)
		end

		io.input(io.popen(shared_state.CANDIDATE_NEIGHBORS_BIN))
		for line in io.lines() do
			table.insert(urls, self:getSyncUrl(line, self.dataType))
		end
	end
...
function SharedState:insert(data, bleachTTL)
	self:lock()
	self:load()
	for key, lv in pairs(data) do
		self:_insert(key, lv, bleachTTL)
	end
	self:save()
	self:unlock()
	self:notifyHooks()
			...
			if lv == nil or lv.bleachTTL < rv.bleachTTL then
				self.log(
					...
					"Updating entry for: "
						.. key
						.. " older: "
						.. (lv and lv.bleachTTL or "no entry")
						.. " newer: "
						.. rv.bleachTTL
				)
				self.storage[key] = rv
...
function SharedState:_remove(key)
	if self.storage[key] ~= nil and self.storage[key].data ~= nil then
		self:_insert(key, nil)
	end
end
...
function SharedStateMultiWriter:_merge(stateSliceArg)
	...
	for key, rv in pairs(stateSlice) do
		local lv = self.storage[key]
		if lv == nil or lv.changes < rv.changes or (lv.changes == rv.changes and lv.fortune < rv.fortune) then
			self.log(
				"debug",
				"Updating entry for: "
					.. key
					.. " older: "
					.. (lv and lv.changes or "no entry")
					.. " newer: "
					.. rv.changes
			)
			self.storage[key] = rv
