					...
					repeat
						...
						if l and l:match("%[in use%]") then
							...
							while count < 5 do
								...
								if channel_noise then
									metric_wifi_survey_channel_noise_dbm(labels, channel_noise)
								end
								if channel_active_time then
									metric_wifi_survey_channel_active_time_ms(labels, channel_active_time)
								end
								if channel_busy_time then
									metric_wifi_survey_channel_busy_time_ms(labels, channel_busy_time)
								end
