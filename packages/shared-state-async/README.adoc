...
Pluggins must be resgistered into shared state by using the config file. UCI infrastracture is prefered 
...
Publishers mus be located at "usr/share/shared-state/publishers"
All Publishers will be called at least once using shared-state-async-publish-all

Sync is called automatically by shared state acording to upgade interval parameter
