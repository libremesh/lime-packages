...
describe("eupgrade tests #eupgrade", function()
	...
	it("test is_new_version_available version is the same as current version", function()
		stub(eup, "_get_board_name", function()
			return "test-board"
		end)
		stub(eup, "_get_current_fw_version", function()
			return "LibreMesh 20.10"
		end)
		stub(eup, "_check_signature", function()
			return true
		end)
	...
	it("test is_new_version_available latest version is not the same as current version", function()
		...
		local status, message = eup.is_new_version_available()
	...
	it("test download_firmware bad hash", function()
		stub(eup, "_file_sha256", function()
			return "faaa"
		end)
		stub(utils, "http_client_get", function()
			return true
		...
		local status, message = eup.download_firmware(latest_data)
	...
	it("test download_firmware ok", function()
		stub(eup, "_file_sha256", function()
			return "fbd95fce091ea10cfa05cfb0ef870da43124ac7c1402890eb8f03b440c57d7b5"
		end)
		stub(utils, "http_client_get", function()
			return true
		end)
	...
	it("test download_firmware while downloading", function()
		eup.set_download_status(eup.STATUS_DOWNLOADING)
		local status, message = eup.download_firmware()
		assert.is_nil(status)
