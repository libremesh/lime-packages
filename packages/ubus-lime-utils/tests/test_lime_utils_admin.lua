...
describe("ubus-lime-utils-admin tests #ubuslimeutilsadmin", function()
	...
	it("test set_root_password", function()
		...
		stub(utils, "set_password", function(pass)
			return pass
		end)

		local response = rpcd_call(ubus_lime_utils, { "call", "set_root_password" }, '{"password": "foo"}')
	...
	it("test firmware_upgrade without new metadata", function()
		...
		stub(utils, "read_file", function()
			return openwrt_release
		end)
		upgrade.set_upgrade_status(upgrade.UPGRADE_STATUS_DEFAULT)
		local response = rpcd_call(ubus_lime_utils, { "call", "firmware_upgrade" }, '{"fw_path": "/foo.bin"}')
		assert.is.equal("ok", response.status)
	...
	it("test last_upgrade_metadata", function()
		...
		stub(utils, "read_obj_store", function()
			return { foo = "bar" }
		end)
	...
	it("test hotspot_wwan_enable default args", function()
		stub(hotspot_wwan, "_apply_change", function()
			return true
		end)
		stub(hotspot_wwan, "enable", function()
			return true
		end)
	...
	it("test hotspot_wwan_enable with args #fooo", function()
		...
		stub(hotspot_wwan, "safe_enable", function()
			return true
		end)
		...
		stub(hotspot_wwan, "disable", function()
			return true
		end)
