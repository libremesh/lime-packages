...
local wireless_service =
	test_utils.load_lua_file_as_function("packages/ubus-lime-utils/files/usr/libexec/rpcd/wireless-service")

local wireless_service_admin =
	test_utils.load_lua_file_as_function("packages/ubus-lime-utils/files/usr/libexec/rpcd/wireless-service-admin")
...
describe("ubus_wireless_service_admin #ubus_wireless_service", function()
	it("get_access_points_data wires to get_access_points_data lib", function()
		...
		stub(wireless, "get_access_points_data", function()
			return mocked_data
		end)
	...
	it("set_node_ap wires to set_node_ap lib", function()
		stub(wireless, "set_node_ap", function()
			return
		end)
		local response = rpcd_call(
			wireless_service_admin,
			{ "call", "set_node_ap" },
	...
	it("set_community_ap wires to set_community_ap lib", function()
		stub(wireless, "set_community_ap", function()
			return
		end)
		local response = rpcd_call(wireless_service_admin, { "call", "set_community_ap" }, '{"enabled": true}')
