...
function node_status.get_station_stats(station)
	...
	local iw_result = utils.unsafe_shell("iw " .. iface .. " station get " .. mac)
	station.rx_bytes = tonumber(string.match(iw_result, "rx bytes:%s+(.-)\n"), 10)
	station.tx_bytes = tonumber(string.match(iw_result, "tx bytes:%s+(.-)\n"), 10)
		...
		for _, role in ipairs(board["switch"]["switch0"]["roles"]) do
			for port_number in string.gmatch(role["ports"], "%S+") do
				if not tonumber(port_number) then
					...
					table.insert(
						response_ports,
						{ num = tonumber(port_number), role = role["role"], device = role["device"] }
					)
				end
			end
		end
