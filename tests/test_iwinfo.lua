...
describe("iwinfo fake tests #iwinfo", function()
	it("test scanlist returning a single station", function()
		...
		local station = iwinfo.fake.scanlist_gen_station("LibreMesh.org", 7, -47, "aa:bb:cc:dd:ee:ff", "Ad-Hoc", 37)
	...
	it("test load_from_uci for a single radio device", function()
		...
		assert.are.same(iwinfo.fake.HWMODE.HW_5GHZ_N, iwinfo.nl80211.hwmodelist("radio0"))
