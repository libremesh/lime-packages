...
local UCI_CONFIG_FILES = {
	...
	"uhttpd",
	"wireless",
	config.UCI_AUTOGEN_NAME,
	config.UCI_NODE_NAME,
	config.UCI_COMMUNITY_NAME,
	config.UCI_DEFAULTS_NAME,
}
...
function utils.rpcd_call(f, script_args, call_args)
	...
	stub(limeutils, "printJson", function(x)
		response = x
	end)
	stub(limeutils, "rpcd_readline", function()
		return call_args
	end)
