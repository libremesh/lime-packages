...
function iwinfo.fake.gen_assoc_station(rx_ht_mode, tx_ht_mode, signal, quality, inactive_ms, tx_packets, rx_packets)
	local utils = require("lime.utils")
...
function iwinfo.fake.load_from_uci(uci_cursor)
	...
	uci_cursor:foreach("wireless", "wifi-device", function(dev)
		create_device(dev)
	end)
end
